...
//			In ra thông tin của tất cả phòng ban (sử dụng toString())
		for (int i = 0; i < departments.length; i++) {
			System.out.println("Department "+ (i+1) + " " + departments[i]);
		}
	}
	public static void question3(Department department) {
		// In ra địa chỉ của phòng ban thứ 1 ****** cai nay co san code ren
	...
	public static void question7(Department[] departments) {
		...
		// order by
		for (int i = 0; i < departments.length; i++) {
			String reverseNameDepartment = reverseWords(departments[i].getName());
			for (int j = 0; j < departments.length - 1; j++) {
				String reverseNameComparingDepartment = reverseWords(departments[j].getName());
		...
		// Xuat ra ky tu dau tien de so sanh
...
//		question2(new Department[] {dep1, dep2, dep3, dep4,dep5});
//		question3(dep1);
//		question4(dep1);
//		question5(dep1, dep2);
//		question6(new Department[] {dep1, dep2, dep3, dep4,dep5});
		question7(new Department[] {dep1, dep2, dep3, dep4,dep5});
	}
}
